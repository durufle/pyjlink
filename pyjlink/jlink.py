...
class JLink(object):
    ...
    # Automatic detection of JTAG speed.
    AUTO_JTAG_SPEED = 0x0
    ...
    def _get_register_index_from_name(self, register) -> int:
        """
    ...
    def disable_dialog_boxes(self):
        ...
        Warning:
          This has the effect of also silencing dialog boxes that appear when updating firmware / to confirm
          updating firmware.

        Dialog boxes will be shown for a brief period of time (approximately five seconds), before being automatically
        hidden, and the default option chosen.
    ...
    def coresight_configure(self,
        ...
        Args:
          ir_pre (int): sum of instruction register length of all JTAG devices in the JTAG chain, close to TDO than the
          actual one, that J-Link shall communicate with

          dr_pre (int): number of JTAG devices in the JTAG chain, closer to TDO than the actual one, that J-Link shall
          communicate with
    ...
    def error(self):
        """
        Shared library internal error state.
    ...
    def clear_error(self):
        """
        Clears the Shared library internal error state.
    ...
    def compatible_firmware_version(self):
        """
        Returns the Shared library's compatible J-Link firmware version.

        Args:
          self (JLink): the ``JLink`` instance

        Returns:
          The firmware version of the J-Link that the DLL is compatible with.
    ...
    def firmware_outdated(self):
        """
        Returns whether the J-Link's firmware version is older than the one that the Shared library  is compatible with.
        ...
        Returns:
          ``True`` if the J-Link's firmware is older than the one supported by the Shared Library, otherwise ``False``.
        """
        date_format = ' %b %d %Y %H:%M:%S'

        date_compatible = self.compatible_firmware_version.split('compiled')[1]
        date_compatible = datetime.datetime.strptime(date_compatible, date_format)

        date_firmware = self.firmware_version.split('compiled')[1]
    ...
    def hardware_info(self, mask=0xFFFFFFFF):
        ...
        Raises:
          JLinkException: on hardware error.
        """
        buf = ctypes.create_string_buffer(32 * 4)
    ...
    def gpio_set(self, pins, states):
        ...
        For each of the given pins, sets the corresponding state based on the index.
    ...
    @connection_required
    def enable_reset_init_registers(self):
        """
    ...
    @connection_required
    def disable_reset_init_registers(self):
        """
    ...
    def set_vector_catch(self, flags):
        ...
        The CPU will jump to a vector if the given vector catch is active, and will enter a debug state.
        This has the effect of halting the CPU as well, meaning the CPU must be explicitly restarted.

        Args:
            flags: vector catch bits
    ...
    def trace_set_format(self, fmt):
        """S
        ets the format for the trace buffer to use.
